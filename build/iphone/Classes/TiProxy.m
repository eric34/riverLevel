...
//Rare exceptions to indicate a bug in the _riverlevel code (Eg, method that a subclass should have implemented)
...
+ (id)createProxy:(NSString*)qualifiedName withProperties:(NSDictionary*)properties inContext:(id<TiEvaluator>)context
{
	...
	if (proxyClass == nil) {
		NSString *_riverlevel = [NSString stringWithFormat:@"%@%s",@"Ti","tanium."];
		if ([qualifiedName hasPrefix:_riverlevel]) {
		...
		if (proxyClass==nil) {
			...
			@throw [NSException exceptionWithName:@"org.riverlevel.module"
